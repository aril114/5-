...
void delTree(TreeNode<T>* node) {
    ...
    if (leftChild != nullptr) delTree(leftChild);
    TreeNode<T>* rightChild = node->getR();
    if (rightChild != nullptr) delTree(rightChild);
    delete node;
}

// копирует дерево
template <typename T>
TreeNode<T>* copyTree(TreeNode<T>* oldTreeRoot) {
    TreeNode<T> *newlptr, *newrptr, *newnode;
    
    if (oldTreeRoot == nullptr)
        return nullptr;
        
    newlptr = copyTree(oldTreeRoot->getL());
    newrptr = copyTree(oldTreeRoot->getR());
...
template <typename T>
int treeCount(TreeNode<T>* root) {
    int size = 0;
    treeCountMain(root, size);
    return size;
}

// увеличивает параметр size на число, равное количеству элементов дерева
template <typename T>
void treeCountMain(TreeNode<T>* node, int &size) {
    TreeNode<T>* leftChild = node->getL();
    if (leftChild != nullptr) treeCountMain(leftChild, size);
    TreeNode<T>* rightChild = node->getR();
    if (rightChild != nullptr) treeCountMain(rightChild, size);
    size++;
}


// удаление узла из дерева с заданным корнем
template <typename T>
void remove(TreeNode<T>* toRemove, TreeNode<T>* root) {
